...
/**
 * Result state shape: { f0, alpha, Q }
 * FFTChart   → sets f0
 * DecayChart → sets alpha, Q
 * Result     → reads all three and classifies
 */
export default function App() {
  const [audioUrl,    setAudioUrl]    = useState(null);
  const [audioBuffer, setAudioBuffer] = useState(null);
  const [result,      setResult]      = useState({});
  const [expMeta,     setExpMeta]     = useState(null); // { dropHeight, surface }

  // Merge partial updates from FFTChart / DecayChart
  const mergeResult = (updater) => {
    setResult((prev) =>
      typeof updater === "function" ? updater(prev) : { ...prev, ...updater }
    );
  };

  // Reset result when a new file is loaded
  const handleBuffer = (buf, meta) => {
    setAudioBuffer(buf);
    setExpMeta(meta ?? null);
    setResult({});
  };
    ...
    >
      ...
      <header className="flex items-end justify-between mb-8 pb-5 border-b border-white/[0.07]">
        <div className="flex items-center gap-4">
          <div
            className="w-11 h-11 rounded-full border border-[#D4AF37] flex items-center justify-center text-[#D4AF37] font-mono text-lg"
        ...
        <div className="flex items-center gap-2 text-[11px] font-mono text-[#7A7870] tracking-widest border border-white/[0.07] rounded-full px-3 py-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-green-400 shadow-[0_0_6px_#4ade80] animate-pulse" />
          {audioBuffer ? "File loaded" : "System ready"}
        </div>
      </header>
      ...
      >
        ...
        <div style={{ gridColumn: 1, gridRow: "1 / 3" }}>
          <Panel title="Input" index="01">
            <Recorder onRecorded={setAudioUrl} onBuffer={handleBuffer} />
          </Panel>
        </div>
        ...
        <div style={{ gridColumn: 2, gridRow: 2 }}>
          <Panel title="FFT Spectrum" index="03">
            <FFTChart audioBuffer={audioBuffer} onResult={mergeResult} />
          </Panel>
        </div>
